...
check_prereq()
{
    ...
    type $dependency >/dev/null 2>&1 || { echo >&2 "Tam Dev requires '$dependency' but it doesn't appear to be installed.  Aborting."; exit 1; }
    ...
    if check_version $installed_version $required_version; then
        ...
        echo "WARNING! Tam Dev did not find the minimum supported version of '$dependency' installed. Required: $required_version, Found: $installed_version"
